...
+ 作用
  ...
  + "none"：不训练任何偏置（推荐）
  + "all"：训练所有偏置-
  + "lora_only"：仅训练 LoRA 适配器的偏置
### task_type

+ 类型：str
+ 作用
  指定微调任务类型，PEFT 会根据任务适配适配器插入逻辑，核心取值：

  + "CAUSAL_LM"（因果语言建模，如 Qwen3 生成）
  + "SEQ_CLS"（文本分类）
  + "QUESTION_ANSWERING"（问答）
  + "SEQ2SEQ_LM"（翻译 / 摘要）|

task_type 告诉 PEFT 框架：

1. 模型要解决的任务类别（生成 / 分类 / 回归等）；
  
2. 应该使用哪种损失函数（如因果语言模型用交叉熵，序列分类用分类交叉熵）；

3. 如何处理模型的输出层（如是否适配分类头、生成时的 token 预测）。

##  进阶可选参数（适配大模型 / 定制化场景）

| 参数名	| 类型	| 作用	| 取值范围 |
|-------------|-------|------------|---------|
| target_modules |	list[str]/str |	指定要插入 LoRA 适配器的模型模块名：<br> - 列表：精准指定模块:<br>"q_proj"：查询<br>"k_proj":键<br>"v_proj"：值<br>"o_proj"：输出<br>"gate_proj"：门控<br>"up_proj"：上采样<br>"down_proj"：下采样<br> - "all-linear"：所有线性层<br> - None：默认适配对应模型的典型模块	 | 模块名列表 / 特殊值 |
